...
@router.post(
    ...
    description="`kind` is one of status|recording|result|summary. The signature is verified "
    "over the RAW request body (see app/integrations/hunar/signature.py); an invalid signature "
    ...
    "to fix. `X-Hunar-Signature` is this project's assumed header name for the signature — "
    "CLAUDE.md documents the HMAC computation itself but not the header carrying it.",
)
